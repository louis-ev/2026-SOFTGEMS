...
- set `public_url` to the canonical URL where SoftGems is accessed by users (emails, selection PDF certificate/video links, QR codes and sharing). Example: `https://softgems.example.com`. Required for correct absolute links in PDFs when Puppeteer runs locally (`localhost`). The legacy `url` field is still accepted as a fallback on the server.
