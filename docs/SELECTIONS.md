...
- **Layout:** invoice-style header (placeholder logo, order / supplier account numbers, counterparty), fixed table (`REF`, composite `Description`, photo, qty, weight, `$/ct`, total), VAT 20%, bank footer from instance field `selection_pdf_bank_footer_en` (`SELECTION_PDF_BANK_FOOTER_EN`, editable by instance admins in the export modal), legal text and ACF footer. Description content uses [pdf_shape_abbreviations.js](../client/src/suggestions/softgems/pdf_shape_abbreviations.js) and [selection_pdf_description.js](../client/src/utils/selection_pdf_description.js) (certificates and videos as links).
