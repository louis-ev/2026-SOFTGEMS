...
- **Layout:** A4 invoice-style document ([`SGSelectionPdfDocument.vue`](../client/src/components/selections/SGSelectionPdfDocument.vue)) matching the "ACF INV N°20265" reference: `210mm` page, `17.8mm` side padding, single `8pt` body size (line-height 1.4), everything in `#1C2B3A`. **Brand:** official ACF logo SVG inlined via [`AcfLogoMark.vue`](../client/src/components/selections/AcfLogoMark.vue) (source: `client/public/images/acf/acf-logo.svg`), ~`18mm` wide and centered; Inter + Crimson Pro via [`acf_brand.css`](../client/public/fonts/acf_brand.css) per ACF Brand Guidelines. **Header:** centered logo, then left block (title with bold document number, `Paris, date`, **N° Commande :**, **N° Compte Fournisseur:**) and counterparty name + address starting at ~72% of the content width. **Table:** `0.5pt` grid borders, fixed column widths from [selection_pdf_columns.js](../client/src/utils/selection_pdf_columns.js) (priced: No 9%, REF 8%, Description 24%, Photo 11%, Quantity 9.5%, Ct weight 10.5%, $/ct 13%, Total 15%); all columns centered except Description (left); `17mm` gem photos. **Totals rows:** `TOTAL` label under No; `VAT` / `Total` labels under Ct weight, `20%` under $/ct. **Number formats** ([selection_pdf_gem_helpers.js](../client/src/utils/selection_pdf_gem_helpers.js)): decimal comma (`2,15`), per-carat without grouping (`7500,00`), currency totals `$16 125,00`. **Payment line:** amount in words via [number_to_words_en.js](../client/src/utils/number_to_words_en.js) — "Please kindly transfer Four hundred… dollars (424 476$US)". VAT 20%, bank footer, legal text (non-priced layouts only) and centered ACF company footer repeated on **every printed page** (`position: fixed` in `@media print`; `thead` repeats, rows avoid page breaks).
