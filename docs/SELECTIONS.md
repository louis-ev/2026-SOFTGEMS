...
- **Folder meta** (in `meta.txt`):
  ...
  - **`notes_before`** / **`notes`**: optional rich-text HTML (bold / italic / link via TextInput editor). Editable in a **Notes** section under Document details (two fields: before / after the gems table). On PDF export, when non-empty, **`notes_before`** is shown just before the gems table and **`notes`** after the gems table (and VAT when present), before the payment line and bank footer — content only, no “Notes” heading. Uses the document body typography. Same content in EN and FR PDF exports (not bilingual-switched).
...
- **Box cover:** only **`box`** selection folders support a cover image (`meta_cover.jpeg`, schema `$cover`). Editable on the box open view header via `CoverField` with **`ratio="free"`** (crop without forced aspect; display keeps the image’s natural proportions). Other selection types have no `$cover` in schema.
