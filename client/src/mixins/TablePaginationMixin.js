...
/** Client-side table pagination with shared localStorage page size. */
export default {
  data() {
    return {
      table_page_index: 0,
      table_page_size: loadTablePageSize(),
    };
  },
  methods: {
    tableEffectivePageSize() {
      return Math.max(1, Number(this.table_page_size) || table_page_size_default);
    },
    ...
    onTablePageSizeChange(next_size) {
      ...
      persistTablePageSize(next_size);
